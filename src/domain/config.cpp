...
bool ParseCamera(const std::string& camera_id,
  ...
  static const std::unordered_set<std::string> kAllowed{
      "record_device", "preview_device", "input_format", "io_mode", "record_width",
  ...
  if (!AssignValue(table, "record_device", &camera->record_device, "camera." + camera_id, err) ||
      !AssignValue(table, "preview_device", &camera->preview_device, "camera." + camera_id, err) ||
  ...
  if (camera->preview_device.empty()) {
    camera->preview_device = camera->record_device;
  }
  return true;
}
...
bool ValidateBoardConfig(const BoardConfig& config, std::string* err) {
  ...
  for (const auto& camera : config.cameras) {
    if (camera.id.empty() || camera.record_device.empty() || camera.preview_device.empty()) {
      if (err) {
        *err = "camera id, record_device and preview_device must not be empty";
      }
      return false;
    }
